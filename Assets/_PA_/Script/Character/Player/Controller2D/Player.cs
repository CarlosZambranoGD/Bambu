...
public class Player : MonoBehaviour, ICanTakeDamage, IListener, ICanFreeze
{
    ...
    [Header("Wall Slide")]
    public bool wallSlideJumpUp = true;
    ...
    public Collider2D checkEnemiesBoxCollider;
    void CheckBelow()
    {
        if (controller.collisions.ClosestHit.collider != null)
        {
            ...
            checkEnemiesBoxCollider.enabled = controller.collisions.ClosestHit.collider.gameObject.layer == LayerMask.NameToLayer("Enemies");
    ...
    void LateUpdate()
    {
        ...
        if(velocity.y > 0 && controller.raycastOrigins.topRight.y > CameraFollow.Instance._max.y)
            velocity.y = 0;
        ...
        if (isWindyZone())
        {
            if (isUsingPatachute)
            {
                if (velocity.y < windyGravity)
                {
                    velocity.y = windyGravity;
                }
            }
        }
        else if (isUsingPatachute)
            velocity.y = parachuteGravity;
    ...
    //        //FallDown();
    //        ControllerInput.Instance.FallDown();
    ...
    void HandleAnimation()
    {
        ...
        anim.SetBool("isUsingPatachute", isUsingPatachute);
        anim.SetBool("isFrozen", isFreezing);
    }
    ...
    #region WINDY ZONE
    [Header("------ WINDY ZONE ------")]
    public float windyGravity = 5;
    public float parachuteGravity = -5;
    //[ReadOnly] public bool isInWindyZone = false;
    [ReadOnly] public bool isUsingPatachute = false;
    public LayerMask windyAsLayer;
    public void SetParachute(bool useParachute)
    {
        if (useParachute && canUseParachute())
            isUsingPatachute = true;
        else
            isUsingPatachute = false;
    }
    ...
    {
        return isWindyZone() || !isGrounded;
    }

    public bool isWindyZone()
    {
        var hit = Physics2D.CircleCast(transform.position + Vector3.up * 0.5f, 0.2f, Vector2.zero, 0, windyAsLayer);
    ...
    [ReadOnly] public bool isFreezing = false;
